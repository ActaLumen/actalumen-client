# ActaLumen Node.js / TypeScript Client

The official SDK for the [ActaLumen](https://actalumen.com) document intelligence platform. ActaLumen uses AI to verify documents against compliance criteria, extract insights, and generate rich media — all through a simple API.

**What you can do with ActaLumen:**
...
```ts
import { ActaLumenClient, waitForDocumentReady, waitForJobReady } from "actalumen-client";
import fs from "fs";

const client = new ActaLumenClient({
  baseUrl: "https://api.actalumen.com",
  apiKey: process.env.ACTALUMEN_API_KEY!,
});
...
All API requests require an API key prefixed with `al_`. Pass it when creating the client:
...
API keys are scoped to an organization and project. The key determines which documents, templates, and resources you can access. Manage your keys from the ActaLumen dashboard under Settings > API Keys.
...
```ts
import { ActaLumenClient, ActaLumenError } from "actalumen-client";
...
```ts
import { waitForDocumentReady, waitForJobReady, downloadToBuffer } from "actalumen-client";
...
The SDK includes a comprehensive integration test suite that exercises every API endpoint. Tests require a valid API key connected to a ActaLumen account with an active subscription.
...
```
ACTALUMEN_API_URL=https://api.actalumen.com
ACTALUMEN_API_KEY=al_your_api_key_here
ACTALUMEN_TEST_PDF=tests/fixtures/nsf-proposal-example.pdf
ACTALUMEN_TEMPLATE_ID=118
```
...
- **`tests/scenarios.ts`** — Shared scenario logic (assertion + API calls). Used by both Vitest tests and the browser-based test runner in the ActaLumen dashboard.
...
ACTALUMEN_API_KEY=al_xxx npx tsx examples/test-documents.ts /path/to/doc.pdf

# Run verification
ACTALUMEN_API_KEY=al_xxx npx tsx examples/test-verification.ts /path/to/doc.pdf [template-id]

# Interactive document chat
ACTALUMEN_API_KEY=al_xxx npx tsx examples/test-chat.ts <document-id>

# Full end-to-end flow
ACTALUMEN_API_KEY=al_xxx npx tsx examples/test-full-flow.ts /path/to/doc.pdf
```

## Requirements

- Node.js >= 18
- A ActaLumen API key (`al_...`)
