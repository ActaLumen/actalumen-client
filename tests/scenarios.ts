 ...
 *
 * Each scenario receives a ActaLumenClient and a test PDF buffer,
 * runs the workflow, and returns collected state for further assertions.
 */

import { ActaLumenClient, ActaLumenError } from "../src/client.js";
...
export async function scenarioUsage(client: ActaLumenClient): Promise<UsageResult> {
...
export async function scenarioInvalidApiKey(
  ...
  const badClient = new ActaLumenClient({
    baseUrl,
    apiKey: "al_invalid_key_00000000000000000000",
    retry: { maxRetries: 0 },
    timeoutMs: 10000,
  });
  ...
  for (const { label, fn } of calls) {
    try {
      ...
      if (err instanceof ActaLumenError) {
