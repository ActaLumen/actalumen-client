 ...
 * Requires ACTALUMEN_TEMPLATE_ID to be set.
 *
 * Run:
 *   ACTALUMEN_API_KEY=al_xxx ACTALUMEN_TEMPLATE_ID=1 npx vitest run tests/verification.test.ts
 */
...
describe("Document Verification", () => {
  ...
  beforeAll(async () => {
    env = getTestEnv();
    if (!env.templateId) throw new Error("ACTALUMEN_TEMPLATE_ID is required for verification tests");
  });
